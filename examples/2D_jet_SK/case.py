    ...
    json.dumps(
        {
            ...
            # -- Patch 1: Cross‐flow (air is now fluid 2) --------------------------
            ...
            # -- Patch 2: Water jet + vapor (water is now fluid 1) -------------
            ...
            # fluid 1 (water) main
            "patch_icpp(2)%alpha_rho(1)": (1.0 - eps - vapor_frac) * rhoW,
            "patch_icpp(2)%alpha(1)": 1.0 - eps - vapor_frac,
            # fluid 2 (air) absent
            ...
            # -- Fluid EOS (now 1=water, 2=air, 3=vapor) ----------------------
            "fluid_pp(1)%gamma": 1.0 / (6.3 - 1.0),   # liquid water
            "fluid_pp(1)%pi_inf": 3.43e8,
            "fluid_pp(2)%gamma": 1.0 / (1.4 - 1.0),   # air
            "fluid_pp(2)%pi_inf": 0.0,
            "fluid_pp(3)%gamma": 1.0 / (1.33 - 1.0),  # water vapor
            "fluid_pp(3)%pi_inf": 0.0,

            # -- Surface tensions ---------------------------------------------
            "sigma": 0.0794,   # between fluid 1 (water) & fluid 2 (air)
            "sigma_2": 0.0588, # between fluid 2 (air) & fluid 3 (vapor)
        }
    )
)
