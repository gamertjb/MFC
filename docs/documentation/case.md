...
| `bubble_inlet_period` | Real    | Time interval between bubble injections |
...
- `bubble_inlet` When true, bubbles leaving the top boundary simply exit the domain while new bubbles from `lag_bubbles.dat` are periodically injected at the bottom.
- `bubble_inlet_period` Sets how often new bubbles are injected when `bubble_inlet` is enabled.
