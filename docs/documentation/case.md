...
- `bubble_inlet` When true, bubbles from `lag_bubbles.dat` are periodically injected at the bottom boundary until `lag_params%nBubs_glb` bubbles have been created. Bubbles rising past the top boundary are left in memory but do not trigger removal.
